...
{-# LANGUAGE LambdaCase #-}
module Transform (transformToCypher, transformToCypherList) where
...
transformToCypherList :: T.Text -> DatabaseQuery -> [T.Text]
transformToCypherList pageLabel dq =
  let queries = concatMap (emitPage pageLabel) (dqResults dq)
  in map strip queries
  where
    strip t = fromMaybe t (T.stripSuffix ";\n" t)
